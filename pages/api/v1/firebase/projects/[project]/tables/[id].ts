...
export default authenticatedMiddleware(async (
	...
	const { id } = req.query;
	if (!id) return res.status(401).json({ "error_msg": `No ${id} parameters found` });

	const response = [];

	if(isFirebase)
	{
		const snapshot = await database.ref('tables').child(id as string).once('value');

		if(snapshot.exists())
		{
			const table: any = snapshot.val();
			table.id = id;
	...
	return res.status(404).json({ 'error_msg': `There is no (new) data available for table ${id}` });
});

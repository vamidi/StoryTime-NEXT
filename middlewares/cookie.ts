...
export const findTokens = async (userId: string, refreshToken: string) =>
{
	...
	const tokens: Tokens[] = await prismaClient.prisma.tokens.findMany({
		where: {
			userId: userId,
		}
	});
...
export const createCookie = async () => {
	...
	const newRefreshTokenExpiry: Date = new Date(
		Date.now() + parseInt(process.env.JWT_EXPIRY) * 1000
	);
...
export const insertToken = async (userId: string, newRefreshTokenHash: string, newRefreshTokenExpiry: Date) =>
{
	...
	const newToken = await prismaClient.prisma.tokens.create({
		data: {
			updated_at: now,
			type: 'API',
	...
	await prismaClient.prisma.users.update({
		where: {
			uid: userId
		},
		data: {
...
export const updateToken = async (token: Tokens, newRefreshTokenExpiry: Date) =>
{
	console.log(token);
	// first create the token in the database then connect it to the user.
	const now = new Date(Date.now());
	await prismaClient.prisma.tokens.update({
		where: {
			uid: token.uid,
		},
		data: {
 ...
 * @param user
 * @param refreshToken
 * @return { token, isMatch, isValid }
 */
export const checkToken = async (user: Users, refreshToken: string) =>
{
	...
	if (token && token.expiration.getTime() > Date.now()) {
		isRefreshTokenValid = true;
		isValid = true;
	}
 ...
 * @param user
 */
export const checkCookie = (fn: NextApiHandler, user: Users) => async (
	req: NextApiRequest,
	res: NextApiResponse
) =>
{
	...
	await insertToken(user.uid, newRefreshTokenHash, newRefreshTokenExpiry);
	return fn(req, res);
};
